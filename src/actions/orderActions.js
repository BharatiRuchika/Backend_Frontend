...
export const createOrder = (order)=>async(dispatch,getState)=>{
    ...
    try{
       ...
       const {data} = await axios.post("http://localhost:3000/order/neworder",order,config);
...
export const myOrders = ()=>async(dispatch,getState)=>{
    ...
    try{
       ...
       const {data} = await axios.get("http://localhost:3000/order/myorders");
...
const {data} = await axios.get(`http://localhost:3000/order/singleorder/${id}`);
       console.log("data",data);
       dispatch({
...
const {data} = await axios.get(`http://localhost:3000/order/admin/orders`);
       console.log("data",data);
       dispatch({
...
export const updateOrder = (id,orderData)=>async(dispatch,getState)=>{
    try{
       ...
       const {data} = await axios.put(`http://localhost:3000/order/admin/orders/${id}`,orderData,config);
...
export const deleteOrder = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`http://localhost:3000/order/admin/orders/${id}`)
