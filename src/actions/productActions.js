...
export const getProducts = (keyword="",currentPage=1,price,category,rating=0)=>async (dispatch)=>{
    ...
    try{
      ...
      let link = `http://localhost:3000/products/getproducts?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&ratings[gte]=${rating}`;
      if(category){
        link = `http://localhost:3000/products/getproducts?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&category=${category}&ratings[gte]=${rating}`
      }
...
export const getProductDetails = (id)=>async (dispatch)=>{
    try{
      ...
      const {data} = await axios.get(`http://localhost:3000/products/getproduct/${id}`);
...
export const newReview = (reviewData)=>async (dispatch)=>{
    ...
    try{
      ...
      const {data} = await axios.put(`http://localhost:3000/products/review`,reviewData,config);
...
export const getAdminProducts = ()=>async (dispatch)=>{
    try{
      ...
      const {data} = await axios.get(`http://localhost:3000/products/admin/getproducts`);
      console.log("actiondata",data.product);
      dispatch({
...
export const newProduct = (productData)=>async(dispatch)=>{
   try{
     ...
     const {data} = await axios.post("http://localhost:3000/products/admin/createProduct",productData,config);
...
export const deleteProduct = (id)=>async(dispatch)=>{
    try{
      ...
      const {data} = await axios.delete(`http://localhost:3000/products/admin/deleteProduct/${id}`);
      console.log("newproductdata",data);
 ...
 export const updateProduct = (id,productData)=>async(dispatch)=>{
    try{
      ...
      const {data} = await axios.put(`http://localhost:3000/products/admin/updateProduct/${id}`,productData);
      console.log("newproductdata",data);
...
export const getProductReviews = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:3000/products/reviews?id=${id}`)
...
export const deleteReview = (id, productId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`http://localhost:3000/products/review?id=${id}&productId=${productId}`)
